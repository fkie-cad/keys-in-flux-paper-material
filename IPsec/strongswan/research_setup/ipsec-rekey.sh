...
side="${1:-}"; ike_id_override="${2:-}"   # ike_id like con1[1]; you may also pass a CHILD name
usage(){ echo "Usage: $0 <left|right> [ike_sa_id|child_name]"; exit 1; }
...
detect_ike_id(){
  # Find an IKE_SA id (e.g., con1[1]) where local-host matches our IP
  ns_exec "swanctl --list-sas --uri '${uri}'" | awk -v ip="$want_ip" '
    /[A-Za-z0-9_.:-]+\[[0-9]+\]:/ { id=$1 }   # header like "con1[1]: ..."
...
[[ -n "$id" ]] || { echo "[ERROR] Could not auto-detect IKE_SA id for local-host ${want_ip}. Provide ike_id or child name." >&2; exit 3; }

echo "[*] Rekey in ns '${ns}' via ${uri} for '${id}' ..."
# If id looks like con[1], treat as IKE; otherwise try as CHILD name.
if [[ "$id" =~ \[[0-9]+\]$ ]]; then
  ns_exec "swanctl --rekey --uri '${uri}' --ike '${id}'"
else
  ns_exec "swanctl --rekey --uri '${uri}' --child '${id}'"
fi
echo "[OK] Rekey command issued."
