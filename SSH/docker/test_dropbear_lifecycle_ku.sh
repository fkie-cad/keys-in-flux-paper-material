#!/bin/bash
#
# Dropbear Lifecycle Test - Key Update (KU) Mode
#
# Tests SSH lifecycle with DATA-DRIVEN rekey:
#   1. Handshake (PRE_CONNECT → KEX_COMPLETE → ACTIVE)
#   2. SSH commands (initial traffic)
#   3. Data transfer (forces automatic rekey after 1GB threshold)
...
# NOTE: Dropbear performs automatic rekey after 1GB of data transfer.
# This test forces rekey by transferring >1GB of data during the session.
#
...
echo ""
echo "========================================================================"
echo "  Dropbear Lifecycle Test - Key Update (KU) Mode"
echo "========================================================================"
echo ""
echo "This test demonstrates SSH key lifecycle with DATA-DRIVEN rekey:"
echo "  - Handshake + KEX (gen_new_keys extracts ChaCha20 key)"
echo "  - Active session with initial commands"
echo "  - Large data transfer to simulate rekey trigger"
echo "  - Post-rekey commands"
echo "  - Session termination"
echo ""
echo "Memory dumps will be captured at each transition."
echo ""
echo "NOTE: Dropbear performs automatic rekey after 1GB of data transfer."
echo "This test uses custom client (dropbear_client_rekey) that simulates"
echo "rekey via large data transfer to match wolfSSH lifecycle pattern."
echo ""
echo "========================================================================"
echo ""
...
PCAP_FILE="${CAPTURES_DIR}/dropbear_lifecycle_ku_$(date +%Y%m%d_%H%M%S).pcap"

# Capture from server container's network namespace
docker run --rm -d --name dropbear_lifecycle_ku_pcap_$$     --net=container:${DROPBEAR_SERVER}     --cap-add NET_ADMIN     -v "${PWD}/${CAPTURES_DIR}:/captures"     nicolaka/netshoot     tcpdump -i any -w /captures/$(basename ${PCAP_FILE}) "port ${SSH_PORT}"     > /dev/null 2>&1 || echo -e "${YELLOW}[PCAP] ⚠️  Could not start capture (non-critical)${NC}"

TCPDUMP_CONTAINER="dropbear_lifecycle_ku_pcap_$$"
sleep 2
...
echo "  - Rekey: ${CYAN}ENABLED${NC} (KU mode - simulated via large data transfer)"
echo ""
...
# NOTE: Using dropbear_client_rekey WITH --with-rekey flag (KU mode - simulates data-driven rekey)
...
timeout 300 docker compose run \
    ...
    -e SSH_CMD="/usr/local/bin/dropbear_client_rekey ${DROPBEAR_SERVER} ${SSH_PORT} ${SSH_USER} ${SSH_PASSWORD} --with-rekey" \
    ${DROPBEAR_CLIENT}
...
echo "  grep -E '_KEX[12]' ${SCRIPT_DIR}/data/keylogs/dropbear_client_keylog.log"
echo ""
