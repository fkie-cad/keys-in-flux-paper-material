...
# OpenSSH will write to /tmp/groundtruth.log (avoids FUSE mount issues)
# Periodically copy from /tmp to the mounted volume
INTERNAL_KEYLOG="/tmp/groundtruth.log"
touch "$INTERNAL_KEYLOG"
chmod 666 "$INTERNAL_KEYLOG"

# Background process to copy keylog from /tmp to mounted volume
(
    while true; do
        if [ -f "$INTERNAL_KEYLOG" ] && [ -s "$INTERNAL_KEYLOG" ]; then
            cp "$INTERNAL_KEYLOG" "$SSHKEYLOGFILE" 2>/dev/null || true
        fi
        sleep 1
    done
) &
...
tail -F "$INTERNAL_KEYLOG" 2>/dev/null &
TAIL_PID=$!
...
cleanup() {
    ...
    # Final copy of keylog before exit
    if [ -f "$INTERNAL_KEYLOG" ]; then
        cp "$INTERNAL_KEYLOG" "$SSHKEYLOGFILE" 2>/dev/null || true
    fi
}
...
if [ "$MODE" = "server" ]; then
    ...
    if [ -f "$SSHD_CONFIG" ]; then
        ...
        # Increase MaxAuthTries for wolfSSH client compatibility (tries multiple auth methods)
        sed -i 's/^#*MaxAuthTries.*/MaxAuthTries 20/' "$SSHD_CONFIG"

        # wolfSSH compatibility mode: ONLY password authentication
        if [ "${AUTH_METHODS:-}" = "password-only" ]; then
            echo "[entrypoint] Configuring password-only authentication for wolfSSH compatibility"
            ...
            # Also disable any AuthenticationMethods directive that might override
            sed -i '/^AuthenticationMethods/d' "$SSHD_CONFIG"
    ...
    # Explicitly set SSHKEYLOGFILE in the environment for sshd
    exec env SSHKEYLOGFILE="$SSHKEYLOGFILE" /usr/sbin/sshd -D -e -p "$SSH_PORT"
