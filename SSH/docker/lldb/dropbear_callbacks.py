...
_wp_env = os.environ.get('LLDB_ENABLE_WATCHPOINTS', 'false')
...
_debugger = None
_process = None
__version__ = "dropbear-cb 2025.10.22-r18"
...
def _set_watchpoint(key_name, address, key_data, key_id):
    ...
    try:
        if not ENABLE_WATCHPOINTS:
            print(f"[aaaaaaaaaaaaaaWATCHPOINT] Watchpoints disabled, skipping set for {key_name}")
            return
        else:
            print(f"[bbbbbbbbbWATCHPOINT] Watchpoints enabled, setting watchpoint for {key_name}")
            ENABLE_WATCHPOINTS = False
            return
    except Exception as e:
        print(f"[cccccccccWATCHPOINT] Exception checking ENABLE_WATCHPOINTS: {e}")
        return
...
def {callback_func_name}(frame, bp_loc, internal_dict):
    ...
    import dropbear_callbacks
    dropbear_callbacks._watchpoints.pop("{fixed_key_name}", None)
...
def fork_callback(frame, bp_loc, internal_dict):
    """Callback when fork() is called - handles Dropbear's double-fork pattern

    Dropbear fork pattern:
    - Fork 1: Parent (PID 1) → Connection handler → KEX happens here → Extract keys
    - Fork 2: Connection handler → Session process → Handler exits

    Strategy: Follow first fork for key extraction, detach before second fork to preserve session
    """
    global _fork_count, ENABLE_DEBUG_OUTPUT
    ...
    print("\n" + "="*72)
    print(f"[FORK] Fork #{_fork_count} detected in PID {pid}")

    if ENABLE_DEBUG_OUTPUT:
        print(f"[FORK_DEBUG] Thread ID: {tid}")
        print(f"[FORK_DEBUG] Process state: {lldb.SBDebugger.StateAsCString(process.GetState())}")
        print(f"[FORK_DEBUG] Number of threads: {process.GetNumThreads()}")

        # Show stack trace to see where fork is called from
        print(f"[FORK_DEBUG] Call stack:")
        for i in range(min(3, thread.GetNumFrames())):
            frame_i = thread.GetFrameAtIndex(i)
            func_name = frame_i.GetFunctionName() or "unknown"
            pc = frame_i.GetPC()
            print(f"[FORK_DEBUG]   #{i}: {func_name} @ 0x{pc:x}")

    if _fork_count == 1:
        # First fork - this creates the connection handler
        print(f"[FORK] FIRST FORK: Parent → Connection handler")
        print(f"[FORK] LLDB will follow child (connection handler)")
        print(f"[FORK] Key extraction will happen in the child process")
        print("="*72 + "\n")

        if ENABLE_DEBUG_OUTPUT:
            print(f"[FORK_DEBUG] Waiting for child process to be created...")

    elif _fork_count == 2:
        # Second fork - this creates the session process
        print(f"[FORK] SECOND FORK: Connection handler → Session process")
        print(f"[FORK] Connection handler (PID {pid}) will exit after fork")
        print(f"[FORK] Keys already extracted - safe to detach")
        print(f"[FORK] DETACHING to prevent breaking SSH session...")
        print("="*72 + "\n")

        if ENABLE_DEBUG_OUTPUT:
            print(f"[FORK_DEBUG] Process will detach and become independent")
            print(f"[FORK_DEBUG] Session child will continue without LLDB")

        try:
            # Detach from the process before it forks
            # This prevents LLDB from attaching to the session child
            # Note: process.Detach() returns lldb.SBError in some versions, void in others
            # For compatibility, just call Detach() and check if exception occurs
            error = process.Detach()

            # Check if error object was returned (API varies by LLDB version)
            if error and hasattr(error, 'Success'):
                if error.Success():
                    print(f"[FORK] ✓ Successfully detached from PID {pid}")
                    if ENABLE_DEBUG_OUTPUT:
                        print(f"[FORK_DEBUG] Detach successful, session will run normally")
                else:
                    print(f"[FORK] ✗ Detach failed: {error.GetCString()}")
                    if ENABLE_DEBUG_OUTPUT:
                        print(f"[FORK_DEBUG] Error details: {error}")
            else:
                # Detach() returned void - assume success
                print(f"[FORK] ✓ Detached from PID {pid}")
                if ENABLE_DEBUG_OUTPUT:
                    print(f"[FORK_DEBUG] Detach called, session should run independently")

        except Exception as e:
            print(f"[FORK] ✗ Exception during detach: {e}")
            if ENABLE_DEBUG_OUTPUT:
                import traceback
                traceback.print_exc()

    else:
        # Unexpected third fork or more
        print(f"[FORK] WARNING: Unexpected fork #{_fork_count} in PID {pid}")
        print(f"[FORK] Dropbear normally only forks twice")
        print("="*72 + "\n")
...
def __lldb_init_module(debugger, internal_dict):
    ...
    _ac_env = os.environ.get('LLDB_AUTO_CONTINUE', 'false')
    _ac_enabled = _ac_env.lower() in ('true', '1', 'yes')
    ...
    print(f"Environment: LLDB_AUTO_CONTINUE = '{_ac_env}'")
    ...
    print(f"Configuration: Auto-continue {'ENABLED' if _ac_enabled else 'DISABLED (manual mode)'}")
    ...
    print("\n✓ Dropbear callbacks loaded successfully")
    if ENABLE_WATCHPOINTS:
    ...
    if bp_fork.IsValid() and bp_fork.GetNumLocations() > 0:
        ...
        if ENABLE_AUTO_CONTINUE:
            ...
            print(f"✓ Set breakpoint on fork() (bp {bp_fork.GetID()}, {bp_fork.GetNumLocations()} locations) - AUTO-CONTINUE ENABLED")
        else:
            print(f"✓ Set breakpoint on fork() (bp {bp_fork.GetID()}, {bp_fork.GetNumLocations()} locations) - will STOP for inspection")
    ...
    # Automatically continue process if it's stopped (controlled by LLDB_AUTO_CONTINUE env var)
    if ENABLE_AUTO_CONTINUE:
        process = target.GetProcess()
        if process.IsValid():
            state = process.GetState()
            state_name = lldb.SBDebugger.StateAsCString(state)
            print(f"\n[AUTO_CONTINUE] Process state: {state_name}")

            if state == lldb.eStateStopped:
                print("[AUTO_CONTINUE] Process is stopped - automatically continuing...")
                error = process.Continue()
                if error.Success():
                    print("[AUTO_CONTINUE] ✓ Process continued successfully")
                else:
                    print(f"[AUTO_CONTINUE] ✗ Failed to continue: {error.GetCString()}")
            elif state == lldb.eStateRunning:
                print("[AUTO_CONTINUE] Process already running - no action needed")
            else:
                print(f"[AUTO_CONTINUE] Process in state {state_name} - will auto-continue when ready")
        else:
            print("[AUTO_CONTINUE] No process yet - will auto-continue when process starts")
    else:
        print("\n[MANUAL_MODE] Auto-continue DISABLED - you must manually press 'c' to continue")
