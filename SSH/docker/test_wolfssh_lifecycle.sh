#!/bin/bash
#
# wolfSSH Lifecycle Test - Simple Session
#
...
# NO REKEY - wolfSSH does not support rekeying.
#
...
echo ""
echo "========================================================================"
echo "  wolfSSH Lifecycle Test (Simple Experiment)"
echo "========================================================================"
echo ""
echo "This test demonstrates basic SSH key lifecycle with wolfSSH:"
echo "  - Handshake + KEX (6 KDF calls: A-F)"
echo "  - Active session with commands"
echo "  - Session termination (NO REKEY - not supported by wolfSSH)"
echo ""
...
rm -f ${CAPTURES_DIR}/wolfssh_lifecycle_*.pcap
...
echo -e "${YELLOW}[CLIENT]${NC} Using custom client: wolfssh-client-rekey WITHOUT --with-rekey"
...
# NOTE: Using wolfssh-client-rekey WITHOUT --with-rekey flag (base mode)
...
if [ -f "${KEYLOGS_DIR}/wolfssh_client_keylog.log" ] && [ -s "${KEYLOGS_DIR}/wolfssh_client_keylog.log" ]; then
    ...
    # Should have 4 keys (wolfSSH calls wc_SSH_KDF 4 times: A-D for AEAD ciphers)
    # AEAD ciphers (ChaCha20-Poly1305) don't use separate MAC keys (E-F)
    if [ "$KEY_COUNT" -eq 4 ]; then
        echo -e "${GREEN}[KEYLOG] ✓ Expected key count for wolfSSH AEAD session (4 keys: A-D)${NC}"
    elif [ "$KEY_COUNT" -gt 4 ]; then
        echo -e "${YELLOW}[KEYLOG] ⚠️  More than 4 keys - unexpected behavior${NC}"
    else
        echo -e "${YELLOW}[KEYLOG] ⚠️  Fewer than 4 keys - KEX may be incomplete${NC}"
    fi
...
if [ -f "${KEYLOGS_DIR}/wolfssh_client_keylog.log" ] && [ -s "${KEYLOGS_DIR}/wolfssh_client_keylog.log" ]; then
    ...
    if [ -f "${PCAP_FILE}" ] || ls ${CAPTURES_DIR}/wolfssh_lifecycle_*.pcap 1> /dev/null 2>&1; then
