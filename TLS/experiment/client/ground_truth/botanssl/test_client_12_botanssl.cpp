...
int main(int argc, char *argv[]) {
    ...
    auto policy = std::make_shared<TLS12_Policy>();
    ...
    while (!client.is_active() && !client.is_closed()) {
        fd_set readfds;
        FD_ZERO(&readfds);
        ...
        if (FD_ISSET(sockfd, &readfds)) {
            ...
            if (received <= 0) {
                std::cerr << "Connection closed during handshake" << std::endl;
                close(sockfd);
                return 1;
            }
    ...
    if (client.is_active()) {
        std::cout << "Established TLS 1.2 connection." << std::endl;
        ...
        while (!client.is_closed()) {
            ...
            FD_SET(sockfd, &readfds);
            struct timeval timeout = {1, 0};
            int sel = ::select(sockfd + 1, &readfds, nullptr, nullptr, &timeout);
            if (sel > 0 && FD_ISSET(sockfd, &readfds)) {
                ...
                client.received_data(reinterpret_cast<const uint8_t*>(buf), n);
            }
        }
    ...
    std::cout << "Disconnected. Process will remain active for 2 seconds before exiting." << std::endl;
    sleep(2);
