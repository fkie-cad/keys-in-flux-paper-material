...
file_path = "/tmp/libretls/timing_13.csv"
save_interval = 6
auto_exit = True
...
def derive_secrets_callback(frame, bp_loc, internal_dict):
    ...
    if set_watchpoint:
        ...
        if error.Success():
            print(f"Setting watchpoint on data address: {data_ptr:#x}")
...
def {callback_name}(frame, bp_loc, internal_dict):
    ...
    if data_ptr != 0 and data_len > 0:
        ...
        if error.Success():
            ...
            import libretls_cb_13
            libretls_cb_13.logger.queue_for_write(str(hit_time), "{fixed_label}", data_hex)
